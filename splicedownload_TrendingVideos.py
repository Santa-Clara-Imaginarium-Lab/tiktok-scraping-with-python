...
if videoType == 1:
    results = api.by_trending(count=count, custom_did=did)
else:
    results = []
    if videoType == 2:
        for x in hashtags:
            res = api.by_hashtag(x, count=count, custom_did=did)
            for y in res:
                results.append(y)
    elif videoType == 3:
        for x in users:
            res = api.by_username(username=x, count=count, custom_did=did)
            for y in res:
                results.append(y)
    elif videoType == 4:
        for x in sounds:
            ...
            res = api.by_sound(x, count=count, custom_did=did)
            for y in res:
                results.append(y)
